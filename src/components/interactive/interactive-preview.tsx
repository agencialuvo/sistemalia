 ...
 * customer taps them on their own device). Reuses the InteractiveBuilder
 * i18n namespace for its couple of fallback strings so it can be dropped
 * into the composer, the automation builder, and the quick-replies
 * manager without needing its own namespace.
 */
    ...
    >
      ...
      {payload.kind === "buttons" ? (
        ...
        >
          ...
          <span className="truncate">{payload.button_label || t("menuDefault")}</span>
        </button>
      )}
    </div>
  );
