...
import { downloadTemplate, importServices } from "@/lib/services/api";
import type { ImportResult } from "@/lib/validators/service";
...
export function ExcelImportDialog({
  ...
  const analyse = useCallback(
    async (picked: File) => {
      ...
      setAnalysing(true);
      try {
        setPreview(await importServices(picked, true));
      } catch (error) {
          ...
          {preview && !analysing && (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <SummaryCard
                  tone="neutral"
                  value={preview.newCategoryNames.length}
                  label={t("import.newCategories")}
                />
              </div>
              ...
              {preview.newCategoryNames.length > 0 && (
                <div className="rounded-lg border border-border bg-muted/40 p-3">
                  <p className="text-xs font-medium text-foreground">
                    {t("import.willCreateCategories")}
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {preview.newCategoryNames.map((name) => (
